...
bool playerRoll(int rollpoints)
{
	//玩家掷骰子
	int score;
	if (rollpoints == 1)
...
bool computerRoll(int rollpoints)
{
	//电脑掷骰子
	int score;
	if (rollpoints == 1)
...
void mainGame(int modecode, int pr, int cr)
{
	//游戏主函数
	if (modecode == 1)
	{
...
{
	//主程序函数，包含运行时的欢迎界面
	cout << " ____       _ _  ____" << endl << "|  _ \\ ___ | | |/ ___| __ _ _ __ ___   ___ " << endl <<
		"| |_) / _ \\| | | |  _ / _` | '_ ` _ \\ / _ \\" << endl << "|  _ < (_) | | | |_| | (_| | | | | | |  __/" <<
		endl << "|_| \\_\\___/|_|_|\\____|\\__,_|_| |_| |_|\\___|" << endl << endl;
	int modecode = 0;
	cout << "Welcome!" << endl << "Choose a game mode: " << endl << "1:Player first" << endl << "2.Computer first" <<
		endl << "3.Play together" << endl;
	cin >> modecode;
	if (modecode == 1)
		gameplay(modecode);
	else if (modecode == 2)
